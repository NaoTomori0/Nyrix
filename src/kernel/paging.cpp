...
// статические таблицы для простого варианта
__attribute__((aligned(4096))) static uint32_t simple_pd[1024];
__attribute__((aligned(4096))) static uint32_t simple_pt[1024];

void paging_init_simple()
{
    // Identity mapping первых 4 МБ с флагами Present + Writable + User
    for (uint32_t i = 0; i < 1024; ++i)
    {
        simple_pt[i] = (i * 4096) | PAGE_PRESENT | PAGE_WRITABLE | PAGE_USER;
    }
    for (uint32_t i = 0; i < 1024; ++i)
    {
        simple_pd[i] = 0;
